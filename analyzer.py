...
def extract_control_overview(doc):
    ...
    # Derive the control suffix dynamically from the extracted control number.
    ctrl_raw = control_id_data["control_number"]
    match = re.search(r'(\d+)', ctrl_raw)
    ctrl_digits = match.group(1) if match else None

    # Derive the target schema only when it is explicitly configured or a control number was extracted.
    ...
    target_schema = explicit_target_schema or (f"ra_ctrl.ctrl_{ctrl_digits}" if ctrl_digits else None)
