...
def is_deliverable_email(email: str) -> bool:
    """
    Validates if an email address is properly formed and uses a deliverable public/corporate domain.
    Explicitly rejects pseudo-domains like .local, hlaproject.local, and dummy addresses (e.g. admin@hlaproject.local).
    """
    ...
    if domain in BLOCKED_EMAIL_DOMAINS or domain.endswith(".local") or domain.endswith(".internal"):
        return False
    if email.lower() == "admin@hlaproject.local":
        return False
    return True
...
def parse_and_validate_emails(raw_email_str: str) -> list[str]:
    """Parses a comma, semicolon, or newline-separated string into a list of valid, deliverable emails."""
    ...
    for p in parts:
        cleaned = p.strip()
        if cleaned and is_deliverable_email(cleaned):
...
def collect_failure_recipients(schedule=None, project=None, doc=None, additional_emails: str = None) -> list[str]:
    """
    Collects only valid deliverable recipient emails for a failure alert:
    1. Emails explicitly specified in additional_emails or schedule's `notification_emails` field.
    2. If none explicitly specified, falls back to schedule creator or project owner (if valid deliverable email).
    Excludes dummy/invalid domains like @hlaproject.local or .local pseudo-domains.
    """
    recipients = []
    ...
    if recipients:
        return [r for r in recipients if is_deliverable_email(r)]

    # Fallback ONLY if no recipient email was configured and creator has a real deliverable email:
    if schedule and getattr(schedule, "creator", None) and getattr(schedule.creator, "email", None):
        c_email = schedule.creator.email.strip()
        if is_deliverable_email(c_email) and c_email not in recipients:
    ...
    if not recipients and project and getattr(project, "creator", None) and getattr(project.creator, "email", None):
        p_email = project.creator.email.strip()
        if is_deliverable_email(p_email) and p_email not in recipients:
...
def try_linux_system_mail(msg, recipients: list, from_addr: str = "alerts@hlastudio.com") -> tuple[bool, str]:
    """
...
def send_control_failure_alert(run_record, schedule=None, project=None, doc=None,
    ...
    """
    recipients = [r for r in collect_failure_recipients(
        schedule=schedule,
        project=project,
        doc=doc,
        additional_emails=additional_recipients
    ) if is_deliverable_email(r)]

    if not recipients:
        logger.info("[EMAIL ALERT SKIPPED] No deliverable recipient emails specified (invalid/dummy domains like @hlaproject.local filtered). Skipping SMTP dispatch.")
        ...
        try:
            ...
            log_entry = EmailNotificationLog(
                ...
                recipient_emails="[NONE - DUMMY DOMAINS FILTERED]",
                subject=subject,
                body_text=body_text,
                body_html=body_html,
                status="SKIPPED",
                error_message="Alert skipped: No valid deliverable recipient email address configured. Invalid/dummy domains (e.g. @hlaproject.local) are excluded.",
            ...
            db.session.rollback()
            logger.debug(f"Could not persist skipped email notification log: {log_e}")

        return {
            "success": False,
            "status": "SKIPPED",
            "message": "No valid deliverable recipient email addresses found. Dummy domains like @hlaproject.local were filtered out.",
            "recipients": [],
            "delivery_mode": "None",
            "log_id": log_id
        }
    ...
    from_addr = smtp_cfg.get("from_email") or smtp_cfg.get("user") or "alerts@hlastudio.com"
...
def test_smtp_connection(target_email: str, override_config: dict = None) -> dict:
    ...
    from_addr = (base_cfg.get("from_email") or "").strip() or user or "alerts@hlastudio.com"

    if not target_email or not is_deliverable_email(target_email.strip()):
        return {
            "success": False,
            "message": f"Invalid recipient email address: '{target_email}'. Must be a deliverable public/corporate domain (dummy domains like @hlaproject.local or .local are prohibited).",
            "diagnostics": "Recipient validation failed: invalid deliverable domain."
        }
...
def send_password_reset_otp_email(user, otp_code: str, expires_minutes: int = 10) -> dict:
    ...
    target_email = getattr(user, "email", None)
    if not target_email or not is_deliverable_email(target_email):
        err_msg = f"User '{getattr(user, 'username', 'unknown')}' does not have a valid deliverable email address configured."
        return {
            "success": False,
            "status": "INVALID_EMAIL",
            "email": target_email,
