            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                ...
                <span className={`badge-pill ${activeEnv === 'prod' ? 'amber' : 'cyan'}`}>
                  {activeEnv === 'prod' ? 'PRODUCTION / PROD' : 'DEVELOPMENT / DEV'}
                </span>
                {/* Access Mode Badge */}
                <span className={`badge-pill ${activeEnv === 'prod' ? 'danger' : 'green'}`} style={{ marginLeft: '0.5rem' }}>
                  {activeEnv === 'prod' ? 'CONTROLLED / PROTECTED' : 'READ / WRITE'}
                </span>
              </div>
      ...
      <div className="target-status-strip">
        ...
        <div className="status-strip-item">
          ...
          <span className="strip-val">
            Engine: {(targetConfigs[activeEnv]?.db_type || targetForm.db_type).toUpperCase()}, Host: {targetConfigs[activeEnv]?.host || targetForm.host}, Port: {targetConfigs[activeEnv]?.port || targetForm.port}, DB: {targetConfigs[activeEnv]?.database_name || targetForm.database_name}, Schema: {targetConfigs[activeEnv]?.schema_name || targetForm.schema_name || 'public'}
          </span>
          {/* Access Mode Badge */}
          <span className={`badge-pill ${activeEnv === 'prod' ? 'red' : 'green'}`} style={{ marginLeft: '0.5rem' }}>
            {activeEnv === 'prod' ? 'CONTROLLED / PROTECTED' : 'READ / WRITE'}
          </span>
            ...
            <div className="deploy-terminal-card">
              ...
              <div className="terminal-body">
                {deployResult ? (
                  <div>
                    {/* ── Status Header ── */}
                    <div className={`log-line ${deployResult.success ? 'success' : 'error'}`} style={{ marginBottom: '0.1rem' }}>
                      {deployResult.success ? '✓' : '✕'}{' '}
                      <strong>{deployResult.success ? 'SUCCESS' : 'ERROR'}:</strong>{' '}
                      {deployResult.message}
                    </div>

                    {/* ── Stages Panel ── */}
                    {deployResult.stages && deployResult.stages.length > 0 && (
                      <div style={{ marginTop: '0.8rem', borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '0.7rem' }}>
                        <div style={{ color: 'var(--cyan-400)', fontWeight: 700, fontSize: '0.72rem', letterSpacing: '0.06em', marginBottom: '0.45rem', textTransform: 'uppercase' }}>
                          Deployment Stages
                        </div>
                        {deployResult.stages.map((stage, idx) => {
                          ...
                          return (
                            <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.45rem', fontSize: '0.78rem', padding: '0.12rem 0', color }}>
                              ...
                              <span style={{ color: isError || isWarn ? color : '#cbd5e1' }}>{stage.label}</span>
                              {stage.detail && (
                                <span style={{ color: '#94a3b8', fontSize: '0.7rem', marginLeft: '0.2rem' }}>({stage.detail})</span>
                              )}
                            </div>
                          )
                        })}
                      ...
                      return (
                        <div style={{ marginTop: '0.8rem', borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '0.7rem' }}>
                          ...
                          <div style={{ fontSize: '0.72rem', color: '#94a3b8', marginBottom: '0.4rem' }}>
                            Schema: <span style={{ color: recon.schema_status === 'EXISTING' ? '#86efac' : recon.schema_status === 'CREATED' ? '#93c5fd' : '#fbbf24', fontWeight: 600 }}>
                              {recon.schema_status}
                            </span>
                            {deployResult.target_schema && <span> — <code style={{ color: '#cbd5e1' }}>{deployResult.target_schema}</code></span>}
                          </div>
                            ...
                            <table style={{ width: '100%', fontSize: '0.73rem', borderCollapse: 'collapse' }}>
                              <thead>
                                <tr style={{ color: '#64748b', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                                  <th style={{ padding: '0.18rem 0.5rem', fontWeight: 600 }}>Object</th>
                                  <th style={{ padding: '0.18rem 0.5rem', fontWeight: 600 }}>Status</th>
                                  <th style={{ padding: '0.18rem 0.5rem', fontWeight: 600 }}>Detail</th>
                                </tr>
                              </thead>
                              <tbody>
                                {Object.entries(objects).map(([objName, objInfo]) => {
                                  ...
                                  return (
                                    <tr key={objName} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                      ...
                                      <td style={{ padding: '0.18rem 0.5rem', whiteSpace: 'nowrap' }}>
                                        <span style={{ color, fontWeight: 700 }}>{icon} {st}</span>
                                      </td>
                                      <td style={{ padding: '0.18rem 0.5rem', color: '#64748b', fontSize: '0.68rem' }}>
                                        {detail}
                                      </td>
                                    </tr>
                                  )
                    ...
                    {/* ── Provisioned tables (legacy pill list) ── */}
                    {deployResult.deployed_tables && deployResult.deployed_tables.length > 0 && !deployResult.reconciliation && (
                      <div style={{ marginTop: '0.6rem' }}>
                        <div style={{ color: 'var(--cyan-400)', marginBottom: '0.4rem' }}>Provisioned Tables in Target DB:</div>
                  ...
                  <div className="log-placeholder">
                    Ready to validate or deploy. Click "Validate &amp; Dry-Run" to preview schema reconciliation in a rollback transaction, or "Deploy to Target" to provision tables idempotently.
                  </div>
                )}
              </div>
            </div>
                  ...
                  <div className="form-group" style={{ marginBottom: '0.15rem' }}>
                    ...
                    <select
                      ...
                      onChange={(e) => setTargetForm({
                        ...
                        schema_name: e.target.value === 'snowflake' ? (targetForm.schema_name ? targetForm.schema_name : 'PUBLIC') : targetForm.schema_name
                      })}
