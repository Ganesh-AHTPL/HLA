...
def test_full_api_flow():
    ...
    p_res = s.post(f"{base}/api/projects", json={
        "name": "Control-Test Enterprise Architecture",
    ...
    assert co.get("target_schema") == "ra_ctrl.ctrl_test", f"Expected ra_ctrl.ctrl_test, got {co.get('target_schema')}"
    ...
    assert targets["dev"].get("schema_name") == "ra_ctrl.ctrl_test", f"Expected ra_ctrl.ctrl_test, got {targets['dev'].get('schema_name')}"
    ...
    assert art_schema == "ra_ctrl.ctrl_test", f"Expected ra_ctrl.ctrl_test, got {art_schema}"

    # Check generated DDL
    ddl = artifact.get("generated_ddl", "")
    assert 'CREATE SCHEMA IF NOT EXISTS "ra_ctrl.ctrl_test";' in ddl
    assert '"ra_ctrl.ctrl_test".' in ddl
    print("   [OK] Verified DDL contains quoted schema 'ra_ctrl.ctrl_test' and tables")
