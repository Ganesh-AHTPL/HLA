...
def test_dynamic_target_control_schema():
    ...
    assert target_schema == "ra_ctrl.ctrl_23", f"Expected 'ra_ctrl.ctrl_23', got '{target_schema}'"
    ...
    # 3. Test Package Generation for Control-23
    print("\n3. Testing Target Logic Package Generation for Control-23:")
    target_config = {
        "db_type": "postgresql",
        "host": "localhost",
        "port": 5432,
    ...
    assert generated_schema == "ra_ctrl.ctrl_23", f"Expected 'ra_ctrl.ctrl_23', got '{generated_schema}'"

    ddl = package.get("ddl", "")
    print("   - Verifying DDL Schema and Table Statements:")
    assert 'CREATE SCHEMA IF NOT EXISTS "ra_ctrl.ctrl_23";' in ddl, "Schema creation statement missing quotes around dot"
    assert '"ra_ctrl.ctrl_23".' in ddl, "Table definitions missing schema prefix '\"ra_ctrl.ctrl_23\".'"
    print("     [OK] CREATE SCHEMA IF NOT EXISTS \"ra_ctrl.ctrl_23\"; present")
    print("     [OK] \"ra_ctrl.ctrl_23\".table_name prefix present")
    ...
    assert '"ra_ctrl.ctrl_23".ctrl_23_balanced_dataset' in pyspark or 'ra_ctrl.ctrl_23' in pyspark, "PySpark JDBC write missing dynamic schema"
